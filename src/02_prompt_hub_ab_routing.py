...
PROMPT_V1_NAME = "nguyen-duy-trong-rag-prompt-v1"
PROMPT_V2_NAME = "nguyen-duy-trong-rag-prompt-v2"
...
def push_prompts_to_hub(client: Client):
    ...
    try:
        url = client.push_prompt(
            PROMPT_V2_NAME,
            object=PROMPT_V2,
            description="V2: Phong cách chuyên gia phân tích, có cấu trúc (3-5 câu)"
        )
...
def main():
    ...
    client = Client(api_key=config.LANGSMITH_API_KEY) if config.LANGSMITH_API_KEY else Client()
    ...
    for i, question in enumerate(SAMPLE_QUESTIONS):
        ...
        print(f"     A: {str(result['answer'])[:75]}...\n")
