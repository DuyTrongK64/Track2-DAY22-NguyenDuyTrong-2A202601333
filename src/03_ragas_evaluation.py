...
SYSTEM_V1 = (
    "Bạn là trợ lý AI thân thiện. Hãy trả lời ngắn gọn, trực tiếp và rõ ràng (2-4 câu) "
    "dựa trên context được cung cấp. Nếu không tìm thấy thông tin trong context, "
...
SYSTEM_V2 = (
    "Bạn là chuyên gia phân tích thông tin và AI. Đọc kỹ context, xác định các luận điểm "
    "và sự kiện chính xác, sau đó đưa ra câu trả lời có cấu trúc mạch lạc, chuẩn xác và chuyên nghiệp (3-5 câu). "
    "Tuyệt đối tuân thủ context và không suy đoán ngoài dữ liệu được cung cấp.\n\n"
    "Context:\n{context}"
)
...
def run_ragas_eval(rag_results: list, version: str) -> dict:
    ...
    Lưu ý: evaluate() thực hiện nhiều lần gọi LLM → mất vài phút / version.
    """
    from ragas.run_config import RunConfig

    print(f"\n📐 Đang đánh giá RAGAS cho prompt {version} ... (vui lòng chờ ~3-8 phút)")
    ...
    result = evaluate(
        ...
        llm=llm_eval,
        embeddings=emb_eval,
        run_config=RunConfig(max_workers=3, timeout=60, max_retries=5),
    )
    ...
    for key in ["faithfulness", "answer_relevancy", "context_recall", "context_precision"]:
        ...
        if isinstance(raw, list):
            valid_vals = [float(v) for v in raw if v is not None and not (isinstance(v, float) and np.isnan(v))]
            scores[key] = float(np.mean(valid_vals)) if valid_vals else 0.0
        elif hasattr(raw, "__iter__"):
            valid_vals = [float(v) for v in raw if v is not None and not (isinstance(v, float) and np.isnan(v))]
            scores[key] = float(np.mean(valid_vals)) if valid_vals else 0.0
        else:
            scores[key] = float(raw) if (raw is not None and not np.isnan(raw)) else 0.0
